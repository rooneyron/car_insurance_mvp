...
TOOL_CALLS = [
    ...
    [("查保单①", "帮我查保单，保单号 POL20260001"), ("→ 输入身份证", "110101199001011234")],
    [("条款检索", "车损险保障哪些情况？")],
    [("转人工", "我要投诉，转人工")],
]
...
MEMORY_TEST = [
    ...
    [("告诉信息", "我叫张三，身份证 110101199001011234，我的车去年出险了")],
    [("→ 我叫什么(短期记忆)", "我叫什么名字？")],
    # ⚠️ 此处应点「清空对话」按钮，再点下方按钮验证长期记忆
    [("→ 我叫什么+出过险吗(长期记忆)", "我叫什么名字？我出过险吗？")],
]
                ...
                for ri, row in enumerate(MEMORY_TEST):
                    with gr.Row():
                        for ci, (label, full_msg) in enumerate(row):
                            ...
                            all_demo_btns.append((btn, full_msg))
                gr.Markdown("<small>⚠️ 第2步后点「清空对话」再点第3步验证长期记忆</small>", visible=True)
